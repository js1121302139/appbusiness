	...
	created: function() {
		...
		mui.plusReady(function() {
			...
			if(JobAndGroupdata.group != undefined) {
				for(var i = 0; i < JobAndGroupdata.group.length; i++) {
					if(JobAndGroupdata.group[i].groupId==_this.getObj.id)continue;
					_this.groups.push({
		...
		createAccount: function() {
			...
			var sendData = {
				...
				fun_Success: function(data) {
					...
					// 更新团队列表
					mui.fire(plus.webview.currentWebview().opener(), "getTeamList");
			...
			console.log(JSON.stringify(sendData) + "-sendData");
