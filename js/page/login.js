...
var Vue = new Vue({
				...
				created: function() {
					...
					mui.plusReady(function() {
						...
						(Fun_App.getdata("token") != null && Fun_App.getdata("token") != " ") ? Fun_App.openWin('index.html', 'pop-in', ''): '';
						plus.navigator.setStatusBarStyle("light");
						_this.pullMessage();
					})
				},
				methods: {
					...
					login: function() {
						var sendData = {
							...
							fun_Success: function(data) {
								...
								if(data.success) { //登录成功
									...
									Fun_App.storagedata("jobCode", data.data.jobCode); //当前账户的角色等级
									Fun_App.storagedata("shopType", data.data.shopType); //当前商铺的类型
									Fun_App.openWin("index.html", "pop-in", 500);
								} else {
									mui.toast(data.message);
								}
						...
						if(Fun_App.checkObjIsNull(sendData.config, ['clientId']) != false) {
							mui("#buttom").button('loading');
							setTimeout(function(){mui("#buttom").button('reset');}.bind(this),5000);
							Fun_App.ExAjax("login", sendData);
						} else {
							mui.toast('你还有数据填写完整!');
						}
